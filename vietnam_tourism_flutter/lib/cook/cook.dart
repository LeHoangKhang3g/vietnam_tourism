...
class CookPost extends StatefulWidget {
  CookPost({ required this.cook }); 
   @override
  _CookPostState createState() => _CookPostState();
  Cook cook;

}

class _CookPostState extends State<CookPost>{
  ...
  void initState(){
    ...
    likes = MyApp.repository.lstStatus.where((element) => element.typePost=="cook")
    .where((element) => element.postId==widget.cook.id).where((element) => element.typeStatus=="like").toList().length;
    unlikes = MyApp.repository.lstStatus.where((element) => element.typePost=="cook")
    .where((element) => element.postId==widget.cook.id).where((element) => element.typeStatus=="unlike").toList().length;

    status = MyApp.repository.lstStatus.where((element) => element.accountId==MyApp.accountUsed.id).where((element) => element.typePost=="cook")
    .where((element) => element.postId==widget.cook.id).toList();
    ...
    descriptionTitle =  widget.cook.description.length<=100?widget.cook.description:widget.cook.description.substring(0,100)+"...";
  }
  ...
  Widget build(BuildContext context){
    ...
    comments=MyApp.repository.comments.where((element) => element.typePost=="cook")
    .where((element) => element.postId==widget.cook.id).toList();
    ...
    if(!isLoad&&!isSave){
      if(likeBeforeSave!=like&&unlikeBeforeSave!=unlike){
        ...
        if(like){
          ...
          .postChangeStatus(Status(0,MyApp.accountUsed.id,"cook",widget.cook.id,"like"), "true").then((value){
            likeBeforeSave=like;
            unlikeBeforeSave=unlike;  
          ...
          API(url: "http://127.0.0.1:8000/api/change-status")
          .postChangeStatus(Status(0,MyApp.accountUsed.id,"cook",widget.cook.id,"unlike"), "true").then((value){
            likeBeforeSave=like;
            unlikeBeforeSave=unlike;  
            MyApp.repository.statusIsUpdate=false;   
            isSave=false;      
        ...
        child: Container(       
          ...
          child: Column(
            ...
            children: [
              ...
              !visibleComment?Container():CommentPage(typePost: "cook",id: widget.cook.id),
